...
// ForeverTestTimeout is used in testing Forever waits
...
func pollInternal(wait Func, condition ConditionFunc) error {
	done := make(chan struct{})
	defer close(done)
	return For(wait, condition, done)
}
...
func pollImmediateInternal(wait Func, condition ConditionFunc) error {
	done, err := condition()
	if err != nil {
		return err
	}
...
func PollUntil(interval time.Duration, condition ConditionFunc, stopCh <-chan struct{}) error {
	return For(poller(interval, 0), condition, stopCh)
}

// Func creates a channel that receives an item every time a test
// should be executed and is closed when the last test should be invoked.
type Func func(done <-chan struct{}) <-chan struct{}

// For gets a channel from wait(), and then invokes fn once for every value
...
func For(wait Func, fn ConditionFunc, done <-chan struct{}) error {
	c := wait(done)
	for {
		_, open := <-c
		ok, err := fn()
...
// poller returns a Func that will send to the channel every
...
func poller(interval, timeout time.Duration) Func {
	return Func(func(done <-chan struct{}) <-chan struct{} {
		ch := make(chan struct{})
